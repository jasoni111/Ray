...
namespace octTree
{
	...
	{
	public:
		octree(vec3f _min, vec3f _max) : isLeaf(true), min(_min), max(_max)
		{
		}
	...
	void insert(octree* cur_ptr, Geometry* const& obj)
	{
		if (cur_ptr->isLeaf)
		{
		...
		else
		{
			...
			auto min_rate = (objBound.min - cur_ptr->min);
			auto max_rate = (objBound.max - cur_ptr->min);
			auto range = (cur_ptr->max - cur_ptr->min);
			min_rate[0] /= range[0];
			min_rate[1] /= range[1];
			min_rate[2] /= range[2];
			...
			max_rate[1] /= range[1];
			max_rate[2] /= range[2];
			auto diff = min_rate - max_rate;
			if (diff[0] >= 0.5 || diff[1] >= 0.5 || diff[2] >= 0.5)
			{
				//too big to go downward. stay here instead.
			}
		}
	}
